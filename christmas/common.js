...
divTableSelector.addEventListener('change', changeTable)
...
function changeTable(event) {
  ...
  if(select.checked)
  {
    if(select.value == "htmlsection"){
      htmlsection.classList.remove('hide');
      jssection.classList.add('hide');
    }
    else if(select.value == "jssection"){
      htmlsection.classList.add('hide');
...
function initSelect(arr) {
  ...
  for(const m of arr){
    createoption(select, m.who1, m.who1);
    if(m.who2){
      createoption(select,m.who2,m.who2);
    }
  }
}
